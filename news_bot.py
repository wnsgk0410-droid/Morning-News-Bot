...
# 1. 수집할 다수 카테고리/키워드 설정 (주요 이슈, 경제, IT)
keywords = ["종합뉴스", "경제", "IT"]
collected_news = []
seen_links = set()

# 2. 정확히 '어제' 날짜 계산 (실행 시점 기준 전날)
...
for keyword in keywords:
  ...
  if res.status_code == 200:
    ...
    for item in items:
      ...
      # 네이버 뉴스 날짜 포맷(RFC 822)을 파싱하여 날짜만 비교
      pub_date_str = item.get("pubDate")
      if pub_date_str:
        try:
          pub_dt = parsedate_to_datetime(pub_date_str)
          # 전날(어제) 작성된 뉴스인 경우에만 수집 (오늘 올라온 새벽 뉴스나 그 이전 뉴스 배제)
...
# 4. 수집된 모든 뉴스를 제미나이가 읽을 수 있는 텍스트로 병합
raw_news_text = ""
...
# 5. 최신 제미나이 모델을 활용한 종합 큐레이션 프롬프트
...
이 방대한 데이터 중에서 중복되거나 쓸모없는 내용은 과감히 거르고, 대중적으로 가장 중요하고 파급력 있는 핵심 이슈들을 분야별(주요 이슈, 경제, IT 등)로 골고루 총 8~10개 엄선해서 종합 브리핑해 줘.

[작성 규칙]
1. 각 뉴스는 반드시 글머리 기호(-)를 사용하여 딱 '3줄'로 핵심만 압축해서 요약할 것.
2. 뉴스 원문 링크나 URL은 절대 포함하지 말 것.
3. 비슷한 성격의 기사나 중복 이슈는 하나로 강력하게 통합할 것. 묶을시 기사 제목을 적절하게 변경.
...
# 디스코드 메시지 최대 글자 수(2000자) 대응
if len(final_message) > 1900:
  final_message = (
      final_message[:1900]
      + "\n\n...(내용이 길어 일부 생략되었습니다. 핵심만 출력되었습니다.)"
  )
